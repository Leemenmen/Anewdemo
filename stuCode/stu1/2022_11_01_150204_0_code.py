...
for year in range(2015, 2017):
    for month in range(8, 12):
        if year <= 2016:
            ...
            if month < 10:
                urls.append(
                    'http://tianqi.2345.com/t/wea_history/js/%s0%s/58362_%s0%s.js' % (year, month, year, month))
            else:
                urls.append(
...
for url in urls:
    ...
    if len(aqi) == 0:
        ...
        info.append(pd.DataFrame({'ymd': ymd, 'high': high, 'low': low, 'tianqi': tianqi,
                    'fengxiang': fengxiang, 'fengli': fengli, 'aqi': aqi, 'aqiInfo': aqiInfo, 'aqiLevel': aqiLevel}))
    else:
        info.append(pd.DataFrame({'ymd': ymd, 'high': high, 'low': low, 'tianqi': tianqi,
                    'fengxiang': fengxiang, 'fengli': fengli, 'aqi': aqi, 'aqiInfo': aqiInfo, 'aqiLevel': aqiLevel}))
    time.sleep(seconds)
...
# 数据导出
weather.to_csv('F:/project/Anewdemo/stuCode/stu1/2022_11_01_150204_csv_0.csv', index=False)
